...
async def handle_force_sub(bot: Client, update: Message):
    if Config.BIMBO_UPDATES_CHANNEL and str(Config.BIMBO_UPDATES_CHANNEL).startswith("-100"):
        channel_chat_id = int(Config.BIMBO_UPDATES_CHANNEL)
    elif Config.BIMBO_UPDATES_CHANNEL and (not Config.BIMBO_UPDATES_CHANNEL.startswith("-100")):
    ...
    try:
        ...
        try:
            ...
            print(f"Unable to do Force Subscribe to {Config.BIMBO_UPDATES_CHANNEL}\n\nError: {err}")
            return 200
        await bot.send_message(
            chat_id=update.from_user.id,
